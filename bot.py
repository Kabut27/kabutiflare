...
async def deploy_webapp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.message:
        ...
        ctx.user_data["dep_webapp"] = "" if text.lower() in ("/skip", "skip") else text
    else:
        ctx.user_data["dep_webapp"] = ""
    return await do_deploy(update, ctx)

def _run_deploy_ssh(host, port, user, auth, password, key_text, bot_token, webapp_url):
    """
    Connect via SSH, write a launcher script, run it with nohup in the background.
    Returns immediately — does NOT wait for Docker to finish installing.
    """
    import time
    ...
    # Write launcher script using printf to avoid heredoc quoting issues
    launcher_lines = [
        "#!/bin/bash",
        f"export BOT_TOKEN='{bot_token}'",
        f"export WEBAPP_URL='{webapp_answer}'",
        "curl -fsSL https://raw.githubusercontent.com/Kabut27/kabutiflare/main/install.sh -o /tmp/kf_install.sh",
        f"printf '1\n{bot_token}\n{webapp_answer}\n' | bash /tmp/kf_install.sh >> /tmp/kabutiflare_install.log 2>&1",
        "echo $? > /tmp/kabutiflare_exit_code",
    ]
    launcher_content = "\n".join(launcher_lines)

    write_cmd = f"printf '%s\n' {chr(39)}{launcher_content}{chr(39)} > /tmp/kf_launcher.sh && chmod +x /tmp/kf_launcher.sh"
    stdin, stdout, stderr = ssh.exec_command(write_cmd, timeout=15)
    stdout.channel.recv_exit_status()

    # Launch in background — nohup ensures it keeps running after SSH closes
    stdin, stdout, stderr = ssh.exec_command(
        "nohup bash /tmp/kf_launcher.sh > /tmp/kabutiflare_install.log 2>&1 </dev/null &",
        timeout=10
    )
    stdout.channel.recv_exit_status()

    # Short wait then verify process is running
    time.sleep(4)
    stdin, stdout, stderr = ssh.exec_command(
        "pgrep -fa 'kf_launcher'  | head -3 || echo NOT_RUNNING",
        timeout=10
    )
    proc_check = stdout.read().decode().strip()
    ssh.close()
...
async def do_deploy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        proc_check = await loop.run_in_executor(
            None,
            _run_deploy_ssh,
            host, port, user, auth, password, key_text, bot_token, webapp_url
        )

        if "NOT_RUNNING" not in proc_check:
            msg_text = (
                "🚀 <b>Install launched in background!</b>\n\n"
                f"🖥 Server: <code>{host}:{port}</code>\n"
                "🐳 Docker install is running — takes <b>3-5 minutes</b>.\n\n"
                "<b>Check progress on server:</b>\n"
                "<code>tail -f /tmp/kabutiflare_install.log</code>\n\n"
                "<b>When done, check Docker:</b>\n"
                "<code>docker compose -f /opt/kabutiflare/docker-compose.yml ps</code>\n"
                "<code>docker compose -f /opt/kabutiflare/docker-compose.yml logs -f</code>"
            )
            await wait.edit_text(msg_text,
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[
            ...
            await wait.edit_text(
                "⚠️ <b>Connected but launcher may not have started.</b>\n\n"
                "Check on server:\n"
                "<code>cat /tmp/kabutiflare_install.log</code>\n\n"
                "Or run manually:\n"
...
def main():
    ...
    deploy_conv = ConversationHandler(
        ...
        states={
            ...
            DEP_WEBAPP: [
                MessageHandler(filters.TEXT, deploy_webapp),
            ],
        },
