            ...
            for rule in rules:
                ...
                row = [
                    InlineKeyboardButton(
                        ...
                        callback_data=f"emtogr_{zid}_{tag}"
                    ),
                    InlineKeyboardButton("🗑 Delete", callback_data=f"emdel_{zid}_{tag}"),
                ]
                btns.append(row)
        else:
...
async def email_toggle_rule(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    uid = q.from_user.id
    # callback: emtogr_{zid}_{tag}
    parts = q.data.replace("emtogr_", "").split("_", 1)
    zid, tag = parts[0], parts[1]

    wait = await q.message.edit_text("⏳ Updating rule…")
    try:
        # Fetch current rule
        rules_d = cf_get(uid, f"/zones/{zid}/email/routing/rules")
        rule = next((r for r in rules_d["result"] if r.get("tag") == tag or r.get("id") == tag), None)
        if not rule:
            await wait.edit_text("❌ Rule not found.", parse_mode="HTML")
            return
...
async def email_delete_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    q = update.callback_query
    await q.answer()
    # callback: emdel_{zid}_{tag}
    parts = q.data.replace("emdel_", "").split("_", 1)
    zid, tag = parts[0], parts[1]
    uid = q.from_user.id
    ...
    if rule:
        ...
        detail = f"<code>{from_v}</code> → <code>{to_v}</code>"
    else:
        detail = f"<code>{tag}</code>"
    ...
    await q.message.edit_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🗑 Yes, Delete", callback_data=f"emdelx_{zid}_{tag}"),
...
async def email_delete_execute(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    uid = q.from_user.id
    parts = q.data.replace("emdelx_", "").split("_", 1)
    zid, tag = parts[0], parts[1]
