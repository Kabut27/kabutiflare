...
async def do_deploy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await wait.edit_text(
            f"✅ Connected to <code>{host}</code>\n\n⏳ Writing config and running installer...",
            parse_mode="HTML"
        )

        # install.sh asks 3 interactive prompts:
        #   1) Install method: 1=Docker, 2=Systemd  -> we choose 2 (Systemd)
        #   2) Bot Token
        #   3) Webapp URL
        # We pipe answers via printf so no interactive terminal is needed.
        ...
        stdin, stdout, stderr = ssh.exec_command(install_cmd, timeout=300, get_pty=True)

        # Stream output back in chunks, update message every ~5 seconds
        import time
        ...
        while True:
            ...
            if line:
                ...
                output_lines = output_lines[-8:]  # keep last 8 lines
                now = time.time()
        ...
        if exit_status == 0:
            await wait.edit_text(
                ...
                f"🟢 Bot is now running via systemd\n\n"
            ...
            err_preview = err_output[-300:] if err_output else "Unknown error"
            await wait.edit_text(
                f"❌ <b>Install script failed (exit {exit_status})</b>\n\n"
                f"<code>{err_preview}</code>\n\n"
                f"Run manually on server:\n"
                f"<code>export BOT_TOKEN='{bot_token}'</code>\n"
                f"<code>bash &lt;(curl -fsSL https://raw.githubusercontent.com/Kabut27/kabutiflare/main/install.sh)</code>",
                parse_mode="HTML"
            )
