...
async def do_deploy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await wait.edit_text("⏳ Connected. Installing dependencies...", parse_mode="HTML")

        # Read current bot.py
        with open(os.path.abspath(__file__), "r") as f:
            bot_code = f.read()

        # Replace tokens in the deployed copy
        bot_code = bot_code.replace(
            f'os.getenv("BOT_TOKEN", "")',
            f'os.getenv("BOT_TOKEN", "{bot_token}")'
        )

        commands = [
            "apt-get update -qq && apt-get install -y python3 python3-pip -qq",
            "pip3 install python-telegram-bot paramiko requests --quiet",
            "mkdir -p /opt/kabutiflare",
        ]
        ...
        await wait.edit_text("⏳ Uploading bot files...", parse_mode="HTML")

        sftp = ssh.open_sftp()
        with sftp.open("/opt/kabutiflare/bot.py", "w") as f:
            f.write(bot_code)

        # Create .env file
        env_content = f'BOT_TOKEN="{bot_token}"\nWEBAPP_URL="{webapp_url}"\n'
        with sftp.open("/opt/kabutiflare/.env", "w") as f:
            f.write(env_content)
...
[Service]
Type=simple
WorkingDirectory=/opt/kabutiflare
ExecStart=/usr/bin/python3 /opt/kabutiflare/bot.py
Restart=always
RestartSec=10
...
"""
        with sftp.open("/etc/systemd/system/kabutiflare.service", "w") as f:
            f.write(service)
        sftp.close()

        await wait.edit_text("⏳ Starting service...", parse_mode="HTML")

        start_cmds = [
            "systemctl daemon-reload",
            "systemctl enable kabutiflare",
            "systemctl restart kabutiflare",
        ]
        ...
        # Check status
        stdin, stdout, stderr = ssh.exec_command("systemctl is-active kabutiflare")
        ...
        if status == "active":
            await wait.edit_text(
                ...
                f"🟢 Service: <b>active (running)</b>\n\n"
                f"<i>Bot is now running on your server via systemd.</i>\n\n"
                f"Useful commands:\n"
                f"<code>systemctl status kabutiflare</code>\n"
                f"<code>journalctl -u kabutiflare -f</code>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[
            ...
            await wait.edit_text(
                f"⚠️ <b>Deployed but service status: {status}</b>\n\n"
                f"Check logs with:\n<code>journalctl -u kabutiflare -f</code>",
                parse_mode="HTML"
            )

    except paramiko.AuthenticationException:
        await wait.edit_text(
            "❌ <b>Authentication failed!</b>\n\nCheck your password or SSH key.",
            parse_mode="HTML"
        )
    except Exception as e:
        await wait.edit_text(
