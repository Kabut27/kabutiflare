...
async def deploy_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "This will install the bot on a remote Linux server via SSH.\n\n"
        "📋 <b>Requirements:</b>\n"
...
async def deploy_host(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "SSH port (default 22):\n\nIf using port 22, just send <code>22</code>.",
        parse_mode="HTML"
    )
    return DEP_PORT
...
async def deploy_port(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "SSH username (usually <code>root</code>):",
        parse_mode="HTML"
    )
    return DEP_USER
...
async def deploy_user(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔑 Password", callback_data="dauth_pass"),
             InlineKeyboardButton("🔐 SSH Key", callback_data="dauth_key")]
        ])
    )
    return DEP_AUTH
...
async def deploy_auth_method(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if method == "pass":
        await q.message.edit_text("🔑 Enter SSH password:")
        return DEP_PASS
    else:
        await q.message.edit_text("🔐 Upload your SSH private key file or paste its contents:")
        return DEP_KEY
...
async def deploy_password(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await update.message.reply_text("🤖 Enter the <b>Bot Token</b> for the deployed bot (from @BotFather):", parse_mode="HTML")
    return DEP_BOTTOKEN
...
async def deploy_bottoken(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await update.message.reply_text(
        "🌐 Enter the <b>WebApp URL</b> (Mini App URL):\n\n"
        "If you don't want a Mini App, send <code>skip</code>",
        parse_mode="HTML"
    )
    return DEP_WEBAPP
...
async def deploy_webapp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    wait = await update.message.reply_text(
        f"⏳ <b>Connecting and installing...</b>\n"
        f"🖥 Server: <code>{host}:{port}</code>\n\n"
        f"<i>This may take 2–5 minutes. Please wait...</i>",
        parse_mode="HTML"
    )
    ...
    if result["success"]:
        ...
        if "timed out" in err.lower():
            tip = "\n\n💡 <b>Tip:</b> Check that the server IP and port are correct, and that SSH is accessible."
        elif "authentication" in err.lower():
            tip = "\n\n💡 <b>Tip:</b> Wrong password or SSH key. Try again."
        elif "connection refused" in err.lower():
            tip = "\n\n💡 <b>Tip:</b> SSH port may be wrong or firewall is blocking access."
        await wait.edit_text(
            f"❌ <b>Error:</b>\n<pre>{err}</pre>{tip}",
            parse_mode="HTML"
        )
