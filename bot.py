    ...
    try:
        ...
        if rules:
            ...
            for i, rule in enumerate(rules):
                ...
                text += f"\n{r_icon} <code>{m_val}</code>\n   ↪ <code>{a_val}</code>\n"
                # Use short index — max callback: "emtogr_32chars_99" = well under 64
                row = [
                    InlineKeyboardButton(
                        f"{'⏸ Disable' if rule.get('enabled') else '▶️ Enable'}",
                        callback_data=f"emtogr_{zid}_{i}"
                    ),
                    InlineKeyboardButton("🗑 Delete", callback_data=f"emdel_{zid}_{i}"),
                ]
                btns.append(row)
        else:
