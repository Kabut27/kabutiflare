...
async def cmd_domains(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for idx, z in enumerate(zones):
            ...
            btns.append([InlineKeyboardButton(f"{color} {i} {z['name']}  •  {plan}", callback_data=f"zone_{z['id']}")])
    ...
    try:
        ...
        if rules:
            ...
            for i, rule in enumerate(rules):
                ...
                text += f"\n{color} {r_icon} <code>{m_val}</code>\n   ↪ <code>{a_val}</code>\n"
                # Disable/Enable + Delete buttons per rule
                toggle_label = "⏸ Disable" if rule.get("enabled") else "▶️ Enable"
                row = [
                    InlineKeyboardButton(
                        f"{color} {toggle_label}",
                        callback_data=f"emtogr_{zid}_{i}"
                    ),
                    InlineKeyboardButton(f"🗑 Delete", callback_data=f"emdel_{zid}_{i}"),
                ]
                btns.append(row)
            text += f"\n{DIV}"
        else:
            text += "<i>No forwarding rules yet.</i>\n"
