/*
 * KabutiFlare — CORS Proxy Worker
 * https://github.com/Kabut27/kabutiflare
 *
 * Deploy: Cloudflare Dashboard → Workers → Create Worker → Paste → Deploy
 *
...
async function handleRequest(request) {
  ...
  if (url.pathname === "/" || url.pathname === "") {
    ...
    return new Response(JSON.stringify({ success: true, message: "KabutiFlare Proxy is running" }), { status: 200, headers: rh });
  }
  ...
  if (email) apiHeaders.set("X-Auth-Email", email);
  if (key) apiHeaders.set("X-Auth-Key", key);

  var init = { method: request.method, headers: apiHeaders };
  if (request.method !== "GET" && request.method !== "HEAD") {
    try { var body = await request.text(); if (body && body.length > 0) init.body = body; } catch(e) {}
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: false, errors: [{ message: err.message }] }), { status: 502, headers: eh });
  }
}
